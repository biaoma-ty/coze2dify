...
MAPPING_RULES: dict[IRNodeType, MappingRule] = {
    ...
    IRNodeType.HTTP_REQUEST: MappingRule(
        IRNodeType.HTTP_REQUEST, "http-request", MappingStatus.PARTIAL, "Auth config differs"
    ),
    IRNodeType.CONDITION: MappingRule(
        IRNodeType.CONDITION, "if-else", MappingStatus.PARTIAL, "Operator format differs"
    ),
    ...
    IRNodeType.LOOP_INFINITE: MappingRule(
        IRNodeType.LOOP_INFINITE, "loop", MappingStatus.PARTIAL, "Max count workaround"
    ),
    IRNodeType.BATCH: MappingRule(IRNodeType.BATCH, "iteration", MappingStatus.PARTIAL, "Parallel iteration"),
    IRNodeType.KNOWLEDGE_RETRIEVAL: MappingRule(
        IRNodeType.KNOWLEDGE_RETRIEVAL, "knowledge-retrieval", MappingStatus.PARTIAL, "Dataset IDs differ"
    ),
    IRNodeType.KNOWLEDGE_WRITE: MappingRule(IRNodeType.KNOWLEDGE_WRITE, "knowledge-index", MappingStatus.MAPPED),
    IRNodeType.KNOWLEDGE_DELETE: MappingRule(
        IRNodeType.KNOWLEDGE_DELETE, "http-request", MappingStatus.PARTIAL, "HTTP workaround"
    ),
    IRNodeType.PLUGIN: MappingRule(IRNodeType.PLUGIN, "tool", MappingStatus.PARTIAL, "Manual tool rebinding required"),
    IRNodeType.SUB_WORKFLOW: MappingRule(
        IRNodeType.SUB_WORKFLOW, "tool", MappingStatus.PARTIAL, "Workflow-as-tool pattern"
    ),
    IRNodeType.VARIABLE_AGGREGATOR: MappingRule(
        IRNodeType.VARIABLE_AGGREGATOR, "variable-aggregator", MappingStatus.MAPPED
    ),
    IRNodeType.VARIABLE_ASSIGNER: MappingRule(IRNodeType.VARIABLE_ASSIGNER, "assigner", MappingStatus.MAPPED),
    IRNodeType.TEXT_PROCESSOR: MappingRule(
        IRNodeType.TEXT_PROCESSOR, "template-transform", MappingStatus.PARTIAL, "Jinja2 template"
    ),
    ...
    IRNodeType.CONVERSATION_OP: MappingRule(
        IRNodeType.CONVERSATION_OP, "http-request", MappingStatus.PARTIAL, "HTTP workaround"
    ),
    IRNodeType.MESSAGE_OP: MappingRule(IRNodeType.MESSAGE_OP, "http-request", MappingStatus.PARTIAL, "HTTP workaround"),
    IRNodeType.DATABASE_QUERY: MappingRule(
        IRNodeType.DATABASE_QUERY, "code", MappingStatus.PARTIAL, "Code node workaround"
    ),
