...
def _output_emitter_case(name: str, *, literal: str | None = None) -> CorpusCase:
    if literal is None:
        ...
        expected_terminal = {"answer": literal}
...
_UNSUPPORTED_CASE_DEFS: list[tuple[str, str]] = [
    ...
    ("knowledge_delete", "60"),
    ("output_emitter_literal_supported_duplicate", "13"),
]


def _unsupported_case(name: str, type_id: str) -> CorpusCase:
    ...
    elif name == "output_emitter_literal_supported_duplicate":
        return _output_emitter_case(name, literal="fixed literal")
    else:
...
CORPUS_CASES: list[CorpusCase] = [
    ...
    _python_code_case(),
    _comment_case(),
    *[_unsupported_case(name, type_id) for name, type_id in _UNSUPPORTED_CASE_DEFS],
]
